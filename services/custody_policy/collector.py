...
def collect_safe_state(rpc_url: str, safe_address: str, chain_id: int | None = None) -> SafeState:
    ...
    modules = _walk_modules(contract)

    fallback = _address_from_storage(w3.eth.get_storage_at(address, FALLBACK_HANDLER_STORAGE_SLOT))
    guard = _address_from_storage(w3.eth.get_storage_at(address, GUARD_STORAGE_SLOT))
    module_guard = _address_from_storage(w3.eth.get_storage_at(address, MODULE_GUARD_STORAGE_SLOT))
